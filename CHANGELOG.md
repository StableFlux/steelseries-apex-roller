...
### Added
- **Proper Windows installer.** Released as `apex-roller-setup-vX.Y.Z.exe`
  (replacing the portable zip). Built with Inno Setup. UAC-elevated
  all-users install: lands in `C:\Program Files\ApexRoller`, registers
  in `HKLM\...\Uninstall` so it always appears in Settings > Apps and
  the legacy Programs and Features view. Offers an opt-in "Launch at
  Windows startup" checkbox during install.
- **Full uninstall through Settings > Apps.** Removes the install dir,
  Start Menu entry, Startup-folder shortcut, and the GameSense
  registration. A PowerShell helper (`installer\uninstall-cleanup.ps1`)
  runs as a separate process during uninstall to kill any running tray
  instance and wipe `%LocalAppData%\ApexRoller`.
- **App icon.** Single multi-resolution `.ico` (16-256 px) embedded in
  the .exe, used by the tray, the Settings entry, the installer wizard,
  and Add/Remove Programs. Generated by `tools/make_icon.py` so the
  design is reproducible.

### Known limitations
- A 320-byte `apex-roller.log` can survive in `%LocalAppData%\ApexRoller`
  after uninstall on some Windows configurations -- the elevated
  uninstaller context sees the directory as "delete pending" while
  Windows finishes releasing file handles, so neither `Remove-Item` nor
  scheduling reboot-deletion via `MoveFileEx` lands. The file is
  harmless, doesn't affect reinstall, and is the only residue.
...
[0.1.3]: https://github.com/StableFlux/steelseries-apex-roller/releases/tag/v0.1.3
[0.1.2]: https://github.com/StableFlux/steelseries-apex-roller/releases/tag/v0.1.2
